...
class User extends ActiveRecord implements IdentityInterface
{
    ...
    {
        return $this->hasMany(\app\models\User::className(), ['referido_por' => 'id']);
    }

    public function getCantdCompras($id)
    {
        $sql = "SELECT COUNT(DISTINCT user.id) as cantd_usuarios FROM curso_abonado
        LEFT JOIN user ON curso_abonado.id_usuario = user.id WHERE user.referido_por = '$id'";
        $comando = Yii::$app->db->createCommand($sql)->queryAll();
        return $comando->execute();
    }

    public function getCompras($id_user)
    {
        $sql = "SELECT COUNT(id_usuario) as cantd_usuarios FROM curso_abonado WHERE id_usuario = '$id_user'";
        $comando = Yii::$app->db->createCommand($sql)->queryAll();
        return $comando;
    }
