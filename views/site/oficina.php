...
<div class="site-contact">
    ...
    <div class="container">
        <p>
        <?= SiteController::translate('Invitation link: ') ?> <code><?= Html::a(Url::home(true) . "index.php?r=site/signup&referente=" . Yii::$app->user->identity->username /*. 'site/usuario/' . Yii::$app->user->identity->username*/, Url::toRoute(['site/signup', 'referente' => Yii::$app->user->identity->username])) ?></code>
        </p>
...
<div class="site-contact" style="margin-top: 30px;">
    <h3><?= SiteController::translate('Páginas de Capturas') ?></h3>
    <div class="row">
        <div class="container">
            <?php
            foreach ($capturas as $key => $value) {
                ?>
                <p>
                   <code><?= Html::a(SiteController::translate("Página Captura ") . $value['id'] , Url::toRoute(['pagina-captura/captura-contacto', 'id' => $value['id']]),['target'=>'_blank']) ?></code>
                </p>
            <?php  } ?>
        </div>
    </div>
...
<div class="site-contact" style="margin-top: 30px;">
<!--    <h3> $pagosDataProvider->count > 0 ? SiteController::translate('Crear ticket') : null </h3>-->
    <h3><?=SiteController::translate('Crear ticket'); ?></h3>
    <div style="padding-top: 8px" class="row">
        <div style="border-radius: 8px 8px " class="container">

            <!-- $pagosDataProvider->count > 0 ? Html::a("Crear", Url::toRoute(['ticket/index']),['style'=>'background-color:#0088cc;color:white;padding:10px;font-size: 18px;border-radius: 8px 8px','class'=>'crear' ]) : null-->
            <?= Html::a("Crear", Url::toRoute(['ticket/index']),['style'=>'background-color:#0088cc;color:white;padding:10px;font-size: 18px;border-radius: 8px 8px','class'=>'crear' ]); ?>
...
<div class="site-contact" style="margin-top: 30px;">
    ...
    <div class="row">
        <div class="container" style="margin-top: 30px;">
            <h3><?= SiteController::translate('My Level') ?></h3><br />
            <?= ($html); ?>
        </div>
        <div>
            <div class="col-lg-12">
                <div class="col-lg-3">
                    <?//= var_dump($cajas); ?>
                    <p class="encab-caja-ofic"><?= SiteController::translate('Level') ?> 1</p>
                    <?php
                        if (isset($cajas['caja_1']))
                        {
                            foreach ($cajas['caja_1'] as $pos => $caja)
                            {    							
						?>                            
                                <div style="display: block;" class="alert alert-warning" >
                                <?= SiteController::translate('Step') ?> <?= $pos + 1 ?> <?= SiteController::translate('Complete ') ?> <i class="fa fa-check-square-o"></i>   
								</div>                               
                    <?php   }
                        }
                    ?>
                ...
                <div class="col-lg-3">
                    <p class="encab-caja-ofic"><?= SiteController::translate('Level') ?> 2</p>
                    <?php
                        if (isset($cajas['caja_2']))
                        {
                            foreach ($cajas['caja_2'] as $pos => $caja)
                            {    
						?>                            
                                <div style="display: block;" class="alert alert-warning" >
                                <?= SiteController::translate('Step') ?> <?= $pos + 1 ?> <?= SiteController::translate('Complete ') ?> <i class="fa fa-check-square-o"></i>   
                ...
                <div class="col-lg-3">
                    <p class="encab-caja-ofic"><?= SiteController::translate('Level') ?> 3</p>
                    <?php
                        if (isset($cajas['caja_3']))
                        {
                            foreach ($cajas['caja_3'] as $pos => $caja)
                            {    
						    ?>                            
                                <div style="display: block;" class="alert alert-warning" >
                                <?= SiteController::translate('Step') ?> <?= $pos + 1 ?> <?= SiteController::translate('Complete ') ?> <i class="fa fa-check-square-o"></i>   
                ...
                <div class="col-lg-3">
                    <p class="encab-caja-ofic"><?= SiteController::translate('Level') ?> 4</p>
                    <?php
                        if (isset($cajas['caja_4']))
                        {
                            foreach ($cajas['caja_4'] as $pos => $caja)
                            {    
						?>                            
                                <div style="display: block;" class="alert alert-warning" >
                                <?= SiteController::translate('Step') ?> <?= $pos + 1 ?> <?= SiteController::translate('Complete ') ?> <i class="fa fa-check-square-o"></i>   
    ...
    <div class="row">
        <div class="container" style="margin-top: 30px;">
            ...
            <?= GridView::widget([
                ...
                'columns' => [
                    ...
                    [
                        'attribute' => SiteController::translate('Costo'),
                        'format' => 'raw',
                        'value' => function ($data)
                        {
                            return Html::a($data->producto->precio);
                        },
                    ],
                ],
            ]); ?>
            ...
            <?= GridView::widget([
                ...
                'columns' => [
                    ...
                    [
                        'attribute' => SiteController::translate('Comision'),
                        'format' => 'raw',
                        'value' => function ($data)
                        {
                            return Html::a($data->comision);
                        },
                    ],
                ],
            ]); ?>
        ...
        <div class="container">
            ...
            <div class="col-lg-4" style="margin-bottom: 5px;">
                ...
                <?php
                    if ($comisiones['solicitables_circuito'] == 3000 || count($cajas['caja_4']) >= 3)
                    {
                    ?>
