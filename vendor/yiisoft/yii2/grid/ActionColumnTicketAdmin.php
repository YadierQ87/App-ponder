 ...
 * @since 2.0
 */
class ActionColumnTicketAdmin extends Column
{
    /**
    ...
    {
        if (!isset($this->buttons['cerraradmin'])) {
            $this->buttons['cerraradmin'] = function ($url, $model, $key) {
                $options = array_merge([
                    'title' => Yii::t('yii', 'Cerrar ticket'),
                    'aria-label' => Yii::t('yii', 'Cerrar ticket'),
                    'data-pjax' => '0',
                ], $this->buttonOptions);
                return $model['estado'] == 1 ? Html::a('<span class="glyphicon glyphicon-ban-circle"> </span>', $url, $options) : null ;
            };
        }
